...
def adjointSens(par):
    ...
    for SDstage in range(2, -3, -1):
        ...
        for i in range(len(time)-1):
            ...
            sol_buffer = solve_ivp(fun=lambda t,z: jointBackward(t,z,par_const, SDstage, stateDim, parIndex, ifOn=ifOn), t_span=(times[SDstage+2][[-1-i,-2-i]]),y0=jointState, t_eval=times[SDstage+2][::-1][i:i+2], method='LSODA',rtol=1e-6,atol=1e-6)
...
def forwardSimulation(par, tSpanSimulation, IC, ifTease=True):
    ...
    for i in range(len(times)):
        ...
        if i<1:
            ...
            sol_buffer =  solve_ivp(fun=lambda t,z: F(t,z,par_const,ifOn=True), t_span=(times[i][0],times[i][-1]),y0=state, t_eval=times[i], method='LSODA',rtol=1e-6,atol=1e-6)
