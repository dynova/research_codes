...
def forwardSens(par, stateDim, parIndex, tSpanSimulation, IC):
    ...
    for i in range(len(times)):
        ...
        if i<1:
            sol_buffer =  solve_ivp(fun=lambda t,z: jointF(t,z,par_const,SDstage=SDstage,stateDim=stateDim,parIndex=parIndex,ifOn=False), t_span=(times[i][0],times[i][-1]),y0=jointState, t_eval=times[i], method='RK45',rtol=1e-6,atol=1e-6)
        else:
            sol_buffer =  solve_ivp(fun=lambda t,z: jointF(t,z,par_const,SDstage=SDstage,stateDim=stateDim,parIndex=parIndex), t_span=(times[i][0],times[i][-1]),y0=jointState, t_eval=times[i], method='RK45',rtol=1e-6,atol=1e-6)
