...
for kk in range(200):
    try:
        temp1 = load('Houston'+str(kk)+'-parLog.npy')
        temp2 = load('Houston'+str(kk)+'-logLLog.npy')
        temp3.append(temp1[where(temp2==amax(temp2))[0][0]])
        temp4.append(-max(temp2))
    except:
        pass
...
for ii in range(len(v2)):
    ...
    ax.set_ylim(0,18500)
    ax.set_title('NLL = ' + str(temp4[v2[ii]]), fontsize = 30)
    ax.set_ylabel('Confirmed case counts')
    fig.tight_layout()
    plt.savefig('m'+str(ii)+'.pdf')
    plt.close()