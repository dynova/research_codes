...
lw = 3;
hold all
g1 = plot(Q2vec,PDT1./PDT2,'Color','b','LineStyle','-','LineWidth',lw);
g2 = plot(Pvec,PDT3(:,1),'bo','MarkerSize',10);
hold off
...
title('Processive-Trunk Model','FontWeight','bold','FontSize',20)
box off
export_fig PDTA -pdf -painters -transparent

figure(2)
lw = 3;
hold all
h1 = plot(Q2vec,PDT2,'Color','b','LineStyle','-','LineWidth',lw);
h2 = plot(Pvec,PDT3(:,2)./10,'bo','MarkerSize',10);
hold off
set(gcf,'units','inches','PaperPosition',[0 0 2.26 1.59])
ax = gca;
set(ax,'FontSize',14,'FontWeight','bold','LineWidth',3,...
...
title('Processive-Trunk Model','FontWeight','bold','FontSize',20)
box off
export_fig PDTB -pdf -painters -transparent