function ydot = EquationsPDT2(t,y)
...
ydot(1) = Q1+Moff*MS1(1)-(Mon*M+delta1)*S1(1)+Dcat*firstrunsumDS1;
ydot(2) = Moff*MS1(2)+Doff*DS1(1)-(Mon*M+Don*D+delta1)*S1(2);
ydot(3) = Moff*MS1(3)+Doff*DS1(2)-(Mon*M+Don*D+delta1)*S1(3);
ydot(4) = Moff*MS1(4)+Doff*DS1(3)-(Mon*M+Don*D+delta1)*S1(4);
ydot(5:l) = Moff*MS1(5:l)+Doff*DS1(4:l-1)-(Mon*M+Don*D+delta2)*S1(5:l);
...
ydot(2*l+6) = Don*D*S1(5)-(Doff+Dcat+delta2)*DS1(4);
ydot(2*l+7:3*l+1) = Don*D*S1(6:l)-(Doff+Dcat+delta2)*DS1(5:l-1);
ydot(3*l+2) = Don*D*S1(l+1)-(Doff+Dcat+delta2)*DS1(l);
ydot(3*l+3) = Q2+Moff*MS2(1)-(Mon*M+delta1)*S2(1)+Dcat*firstrunsumDS2;
ydot(3*l+4) = Moff*MS2(2)+Doff*DS2(1)-(Mon*M+Don*D+delta1)*S2(2);
ydot(3*l+5) = Moff*MS2(3)+Doff*DS2(2)-(Mon*M+Don*D+delta1)*S2(3);
ydot(3*l+6) = Moff*MS2(4)+Doff*DS2(3)-(Mon*M+Don*D+delta1)*S2(4);
ydot(3*l+7:4*l+2) = Moff*MS2(5:l)+Doff*DS2(4:l-1)-(Mon*M+Don*D+delta2)*S2(5:l);
...
ydot(5*l+8) = Don*D*S2(5)-(Doff+Dcat+delta2)*DS2(4);
...
ydot(6*l+6) = (Doff+Dcat+delta1)*(DS1(1)+DS1(2)+DS1(3)+DS2(1)+DS2(2)+DS2(3))+...
    (Doff+Dcat+delta2)*(secondrunsumDS1+secondrunsumDS2)-Don*D*(secondrunsumS1+secondrunsumS2);
end