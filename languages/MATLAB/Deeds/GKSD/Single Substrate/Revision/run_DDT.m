...
for i = 1:n
    ...
    [DDT1, DDT2] = DDT(Sinit,delta1,aM,aD);
    v1 = DDT1./DDT2;
    v2 = DDT2;
...
for series = 1:len
    ...
    [t,y] = ode15s(@EquationsDDT,0:1e5:1e6,y0,odeset('RelTol',1e-5,'AbsTol',1e-10));
    DDT1(series) = sum(y(end,[5:l+1,l+6:2*l+1,2*l+5:3*l+1]));
    DDT2(series) = sum(y(end,1:3*l+1));
end

    function ydot = EquationsDDT(t,y)
        S = y(1:l+1);
        MS = y(l+2:2*l+1);
        DS = y(2*l+2:3*l+1);
        M = y(3*l+2);
        ...
        ydot(1) = Q-delta1*S(1)+dM*MS(1)-aM*M*S(1)+kcatD*firstrunsumDS;
        ydot(2) = -S(2)*(delta1+aM*M+aD*D)+kcatM*MS(1)+dD*DS(1)+dM*MS(2);
        ydot(3) = -S(3)*(delta1+aM*M+aD*D)+kcatM*MS(2)+dD*DS(2)+dM*MS(3);
        ydot(4) = -S(4)*(delta1+aM*M+aD*D)+kcatM*MS(3)+dD*DS(3)+dM*MS(4);
        ...
        %% Equations for K and P
        ydot(3*l+2) = (dM+kcatM+delta1)*(MS(1)+MS(2)+MS(3)+MS(4)) + (dM+kcatM+delta2)*runsumMS - ...
            aM*M*firstrunsumS;
        ydot(3*l+3) = (dD+kcatD+delta1)*(DS(1)+DS(2)+DS(3)) + (dD+kcatD+delta2)*secondrunsumDS - ...
            aD*D*secondrunsumS;
        %% Equations for S4, S5, ..., S_(l-1) and MS4, MS5, ..., MS_(l-1)
        ydot(5:l) = -S(5:l)*(delta2+aM*M+aD*D)+kcatM*MS(4:l-1) + ...
