...
for i = 1:n
    ...
    [PDS1, PDS2] = PDS(Sinit,delta1,Mon,Don);
    v1 = PDS1./PDS2;
    v2 = PDS2;
...
save PDS.mat out_r50 out_neff

function [PDS1, PDS2] = PDS(Sinit,delta1,Mon,Don)
l = 50;
len = 100;
delta2 = 10*delta1;
Q = Sinit*delta1;
...
for series = 1:len
    ...
    [t,y] = ode15s(@EquationsPDS,0:1e5:1e6,y0,odeset('RelTol',1e-5,'AbsTol',1e-10));
    PDS1(series) = sum(y(end,[5:l+1,l+6:2*l+2,2*l+6:3*l+2]));
    PDS2(series) = sum(y(end,1:3*l+2));
end
